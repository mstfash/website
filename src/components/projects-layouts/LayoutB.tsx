...
const LayoutB: React.FC<LayoutAProps> = ({ project, onBackClick }) => {
  return (
        ...
        <div className="md:p-8">
            ...
            project.fields?.projectImages?.[index]?.fields?.file?.url ? (
              <div
                ...
                className={`h-full w-full p-10 ${
                  index === 0 ? 'md:w-3/4 mr-auto' : ''
                } ${index === 1 ? 'md:w-3/4 ml-auto' : ''}`}
              >
                <img
          ...
          {project.fields?.projectImages?.[2]?.fields?.file?.url &&
            project.fields?.projectImages?.[3]?.fields?.file?.url && (
              <div className="flex w-full md:flex-nowrap flex-wrap p-10">
                {[3, 2].map((index) => {
                  const imageUrl =
                    project?.fields?.projectImages?.[index]?.fields?.file?.url;
                  ...
                  return (
                    <div
                      key={index}
                      className={`w-full md:w-1/2 md:p-2 pt-4 flex ${
                        index === 3 ? 'items-end' : ''
                      }`}
                    >
                      <img
                        ...
                        style={
                          index === 3 ? { height: '300px' } : { height: 'auto' }
                        }
                      />
                    </div>
                  );
          ...
          {(project.fields?.projectImages?.[6]?.fields?.file?.url ||
            ...
            <section className="flex flex-col md:flex-row gap-6 md:gap-10 md:px-10 h-auto md:h-[90vh]">
              {project.fields.projectImages[6]?.fields?.file?.url && (
          ...
          {project.fields?.projectImages?.[7]?.fields?.file?.url && (
            <div className="h-full w-full md:p-8 p-0 m-0">
              <img
