...
function LayoutI({ project, onBackClick }: LayoutAProps) {
  return (
      ...
      <div className="w-full ">
          ...
          {project.fields?.heroImage?.fields?.file?.url && (
            <>
              <img
                ...
                className="w-full md:h-full md:object-cover h-auto object-scale-down"
              />
              {/* Text Overlay */}
        ...
        <div className="md:p-8">
            ...
            project.fields?.projectImages?.[index]?.fields?.file?.url ? (
              <div
                ...
                className={`h-full w-full p-10 ${
                  index === 1 ? 'w-full md:w-3/4 mr-auto' : ''
                } ${index === 2 ? 'w-full md:w-3/4 ml-auto' : ''}
                 ${index === 4 ? 'w-full md:w-3/4 mr-auto' : ''}`}
              >
                <img
          ...
          {(project.fields?.projectImages?.[6]?.fields?.file?.url ||
            ...
            <section className="flex flex-col md:flex-row gap-6 md:gap-4  md:px-10 h-auto md:h-[50vh] pt-10 p-10">
              {project.fields.projectImages[6]?.fields?.file?.url && (
