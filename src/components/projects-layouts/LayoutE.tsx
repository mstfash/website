...
function LayoutE({ project, onBackClick }: LayoutAProps) {
  return (
        ...
        <div className="md:p-8 ">
          {[0, 1].map((index) =>
            project.fields?.projectImages?.[index]?.fields?.file?.url ? (
              <div key={index} className="w-full p-10">
                <img
          ...
          {/* صور 2 و 3 جنب بعض */}
          {project.fields?.projectImages?.[2]?.fields?.file?.url &&
            project.fields?.projectImages?.[3]?.fields?.file?.url && (
              <div className="flex flex-wrap md:flex-nowrap w-full gap-4 p-10">
                {[3, 2].map((index) => {
                  const imageUrl =
                    project?.fields?.projectImages?.[index]?.fields?.file?.url;
          ...
          {project.fields?.projectImages?.[4]?.fields?.file?.url &&
            project.fields?.projectImages?.[5]?.fields?.file?.url && (
              <div className="w-full px-10 py-10 gap-4">
                <div className="w-full md:w-1/2 flex justify-end md:ml-28 pb-10">
                  <img
                    src={project.fields.projectImages[4].fields.file.url}
                    alt={`${project.title} - Image 5`}
                    className="object-cover"
                  />
                </div>

                <div className="w-full md:w-full h-full py-10">
                  <img
